    ...
    <main className="pt-16 bg-[#f6f5f1] min-h-screen font-sans">
      <section className="py-24 px-6 border-b border-neutral-200 bg-white">
        <div className="max-w-7xl mx-auto flex flex-col gap-6">
          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#ECB65F]">CALENDAR</span>
                ...
                >
                  ...
                  <div className="p-10 flex flex-col justify-between flex-1">
                    <div className="flex flex-col gap-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col gap-1">
                          <h3 className="text-xl font-bold text-[#44ACFF] uppercase group-hover:text-[#ECB65F] transition-colors">{event.title}</h3>
                          <div className="flex items-center gap-2 text-xs font-bold text-neutral-400 uppercase tracking-widest">
                            <MapPin className="w-3 h-3 text-[#ECB65F]" /> {event.location}
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="mt-8 flex items-center justify-between border-t border-neutral-50 pt-6">
                      <div className="flex items-center gap-2 text-[10px] font-bold text-[#44ACFF] uppercase tracking-widest">
                        <Clock className="w-3.5 h-3.5 text-[#ECB65F]" /> {event.time}
                      </div>
      ...
      {pastEvents.length > 0 && (
        <section className="py-24 px-6 border-t border-neutral-100">
          <div className="max-w-7xl mx-auto">
            <h2 className="text-2xl font-bold text-[#44ACFF] uppercase tracking-widest mb-12">Past Events</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 opacity-60">
              {pastEvents.map(event => (
                <div key={event.id} className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    <h4 className="text-sm font-bold text-[#44ACFF] uppercase tracking-tight">{event.title}</h4>
